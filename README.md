...
- 模式编辑支持轻度与中度锁定；中度锁定必须由用户手动开启无障碍服务。
- 无障碍服务仅监听前台窗口所属应用，不读取界面内容、不自动点击。
- 中度锁定发现非白名单应用后返回 View Clock，并显示被阻止的包名。
- 中度锁定同时监听窗口状态和窗口列表变化，覆盖上划进入最近任务的路径。
- 专注页明确显示“轻度：仅记录”或“中度：自动返回”。
- Usage Access 与无障碍的重复违规事件会去重，避免一次离开被统计两次。
- Android 13+ 增加通知权限状态和系统授权请求；未授权时保留倒计时并停止前台监控。
- 系统设置、系统界面和默认拨号应用始终保留为安全出口。
